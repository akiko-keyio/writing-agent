...
## App overrides (Phase 1)

The writing-agent app **does not** use the multi-font editorial system below.

| Override | Value |
|----------|-------|
| Font (all UI + document + diff) | `"Microsoft YaHei UI", "Microsoft YaHei", sans-serif` |
| Typography authority | [`design-spec.md`](design-spec.md) — sizes only; ignore Copernicus / StyreneB / JetBrains Mono rules here |
| Colors / spacing / radii | Still use this file's `{colors.*}`, `{spacing.*}`, `{rounded.*}` |
