...
def test_defaults_all_enabled(tools_yaml: Path):
    assert load_tool_prefs()["read_document"] is True
    assert "read_document" in get_enabled_tool_ids()
...
def test_disable_and_persist(tools_yaml: Path):
    ...
    assert data["tools"]["read_document"]["enabled"] is False
    assert "read_document" not in get_enabled_tool_ids()
    assert all(t.tool_name != "read_document" for t in get_enabled_writing_tools())


def test_list_tools_for_settings_reflects_state(tools_yaml: Path):
    set_tool_enabled("read_document", False)
    items = list_tools_for_settings()
    read_document = next(item for item in items if item["id"] == "read_document")
    assert read_document["enabled"] is False
