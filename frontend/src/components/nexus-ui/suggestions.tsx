...
const suggestionVariants = cva(
  "h-8 rounded-full text-sm font-normal shadow-none outline-0 transition-all duration-150 focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.99]",
  {
    variants: {
      variant: {
        filled: "border-none bg-muted text-primary",
        outline: "text-primary",
        ghost: "border-none text-muted-foreground",
      },
    },
    defaultVariants: {
      variant: "filled",
...
const suggestionButtonVariant = {
  filled: "secondary",
  outline: "outline",
  ghost: "ghost",
} as const satisfies Record<
  NonNullable<VariantProps<typeof suggestionVariants>["variant"]>,
...
function Suggestion({
  ...
  return (
    <Button
      ...
      variant={suggestionButtonVariant[variant ?? "filled"]}
      className={cn(suggestionVariants({ variant }), gap.sm, p[4].x, className)}
      onClick={(e) => {
        onClick?.(e);
  ...
  return (
    <Presence present={open}>
      <div
        ...
        className={cn(
          cn(
            "rounded-t-0 absolute inset-x-0 -top-7.5 z-0 mx-auto flex w-[calc(100%-16px)] flex-col items-center justify-center rounded-b-lg bg-muted duration-200 data-[state=closed]:animate-out data-[state=closed]:duration-0 data-[state=closed]:fade-out-0 data-[state=closed]:slide-out-to-top-2 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-top-2",
            gap.md,
            p[2].x,
            p[3].y
          ),
...
function SuggestionPanelHeader({
  ...
  return (
    <div
      ...
      className={cn("flex w-full items-center justify-between", p[3].x, className)}
      {...props}
    />
  );
}
